...
from vsr.misc import raise_warn_call
from vsr.pyadm1.basic_classes.Phy import R, p_atm
from vsr.pyadm1.model.vs_computation import (N_DAY_VSR, dig_states_vs, feed_vs,
                                             volume_per_day)
...
def run_adm1(
    ...
    **kwargs,
    ...
    # TODO: 46 should be inferred from basic_classes.state
    simulate_results = np.full(shape=(n_days, 46), fill_value=np.nan)
    ...
    v_liq_d = volume_per_day(influent_state[:, 0], V_liq_s)
    ...
    simulate_results[:, 44] = VS_in
    simulate_results[:, 45] = np.nan  # Fill in first values
    ...
    if n_day_vsr < n_days:
        ...
        simulate_results[:(-n_day_vsr), 45] = VSR
